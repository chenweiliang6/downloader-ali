...
package com.qdapi.downloaderManager.download.core;

import android.os.Handler;
import android.os.Message;

import com.qdapi.downloaderManager.download.DownloadConfig;
import com.qdapi.downloaderManager.download.utils.Logger;
import com.qdapi.downloaderManager.download.utils.TickTack;
...
public class DownloadTaskManager implements ConnectThread.ConnectListener, DownloadThread.DownloadListener {
    ...
    void start() {
        ...
        if (mDownloadEntry.status == DownloadEntry.Status.DOWNLOADING) {
            Logger.w(TAG, "id:" + mDownloadEntry.id + " already start Download! Skip");
            return;
        }
        ...
        if (mDownloadEntry.totalLength > 0) {
            ...
            notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_CONNECTING);
    ...
    private void startMultiDownload() {
        ...
        notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_DOWNLOADING);
    ...
    public void onConnected(boolean isSupportRange, int totalLength) {
        ...
        notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_CONNECT_SUCCESSFUL);
    ...
    public void onConnectError(String message) {
        if (isPaused || isCancelled) {
            ...
            notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_PAUSED_OR_CANCELLED);
        } else {
    ...
    public synchronized void onProgressChanged(int index, int progress) {
        ...
        if (TickTack.getInstance().needToNotify()) {
            notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_UPDATING);
        }
    }
    ...
    public synchronized void onDownloadCompleted(int index) {
        ...
        if (mDownloadEntry.totalLength > 0 && mDownloadEntry.currentLength != mDownloadEntry.totalLength) {
            ...
            notifyUpdate(mDownloadEntry, DownloaderHandler.NOTIFY_COMPLETED);
        }
    }
